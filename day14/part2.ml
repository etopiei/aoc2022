...
let check_rock_at rocks floor (x, y) =
  match y with
  | a when a = floor -> true
  | _ -> Hashtbl.mem rocks (x, y)
;;
...
let simulate_sand rocks = 
  ...
  let floor = Seq.map (fun (x, y) -> y) (Hashtbl.to_seq_keys rocks) |> Seq.fold_left max 0 |> fun x -> x + 2 in
  let sand_ref = ref {x=500; y=0} in
  let rested_sand = ref 0 in

  (* If it goes below floor, quit *)
  while !sand_ref.y != -1 do
    (* Move sand down if possible *)
    match check_rock_at rocks floor (!sand_ref.x, !sand_ref.y + 1) with
    | false -> sand_ref := {x= !sand_ref.x; y=(!sand_ref.y + 1)};
    | true -> match check_rock_at rocks floor (!sand_ref.x - 1, !sand_ref.y + 1) with
      | false -> sand_ref := {x= !sand_ref.x - 1; y= !sand_ref.y + 1};
      | true -> match check_rock_at rocks floor (!sand_ref.x + 1, !sand_ref.y + 1) with
        | false -> sand_ref := {x= !sand_ref.x + 1; y= !sand_ref.y + 1};
        | true -> (if !sand_ref.x = 500 && !sand_ref.y = 0 then (print_int (!rested_sand + 1); exit 0) else Hashtbl.add rocks (!sand_ref.x, !sand_ref.y) true; sand_ref := {x=500; y=0}; rested_sand := !rested_sand + 1);
  done;
;;
