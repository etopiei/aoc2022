...
let worry = List.fold_left (fun a b -> a * b) 1 (List.map (fun m -> m.result_divisible_by) monkeys);;
...
let apply_rules monkey monkey_lst item = 
  ...
  | "**" -> (power item monkey.check_value) mod worry;
  | "*" -> (item * monkey.check_value) mod worry;
  | _ -> (item + monkey.check_value) mod worry in
...
let final_monkeys = do_with_value_n_times monkeys 10000 run_round;;
